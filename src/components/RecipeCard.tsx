...
export default function RecipeCard({
  ...
  return (
    <Link 
      to={`/recipes/${id}`} 
      className="recipe-card group block focus:outline-none focus:ring-2 focus:ring-primary/50 rounded-2xl overflow-hidden bg-card border border-border shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
    >
      {/* Image */}
      <div className="relative overflow-hidden aspect-[4/3] bg-gradient-to-br from-muted to-muted/50">
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
          ...
          {hasSteps && stepCount && (
            <Badge className="bg-background/95 text-foreground shadow-lg font-medium px-3 py-1 backdrop-blur-sm">
              📋 {stepCount} steps
            </Badge>
          )}
        </div>
        ...
        <div className="absolute top-3 right-3 z-10">
          <Button
            size="sm"
            variant="outline"
            className={`rounded-full bg-background/95 backdrop-blur-md shadow-lg border-2 transition-all duration-200 hover:scale-110 ${
              saved 
      ...
      <div className="p-5 space-y-3.5 bg-gradient-to-b from-card to-card/50">
        ...
        {showEdit ? (
          ...
          <Button 
            className="w-full rounded-full font-semibold shadow-sm group-hover:shadow-md transition-all duration-200 group-hover:scale-[1.02]" 
            onClick={(e) => e.preventDefault()}
          >
